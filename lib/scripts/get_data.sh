...
if [[ $ACTIVE_WIDGETS == *"soundWidget"* ]]; then
...
if [[ $ACTIVE_WIDGETS == *"micWidget"* ]]; then
  MIC=$(osascript -e 'set ovol to input volume of (get volume settings)')
fi

if [[ $ACTIVE_WIDGETS == *"spotifyWidget"* ]]; then
...
if [[ $ACTIVE_WIDGETS == *"musicWidget"* ]]; then
  MUSIC_PROCESS="Music"
...
BROWSER_TRACK="{}"
if [[ $ACTIVE_WIDGETS == *"browserTrackWidget"* ]]; then
